...
2. Read this repo's local config for grounding: `base/agent/settings.json` and `base/agent/models.json` (the repo root's `base/` is symlinked to `~/.pi`, so `base/agent/` here _is_ `~/.pi/agent/`). Also list `extensions/`, `lib/`, and `themes/` at the repo root — `extensions/` is project-local/opt-in, kept as a sibling of `base/agent/` on purpose (see `README.md`'s "Extending Pi" section for why); `lib/theme-map.ts` is a shared helper (not an extension) kept out of `extensions/` so Pi's auto-discovery doesn't try to load it; `themes/` is not symlinked anywhere global — each extension registers only its own assigned theme file via `lib/theme-map.ts`'s conditional discovery (see README's "Themes" section). Extensions are launched by name via a `piext()` shell function in `~/.dotfiles/functions_zsh` (not part of this repo), or persistently via `npm run toggle-extensions` — read `piext()` if present for the exact on-demand mechanism.
