 ...
 * - Bash restricted via a real command tokenizer, not a raw-string regex (tool-policy.ts)
 * - Owns its own clarifying-questions tool, plan_mode_question (question-tool.ts)
 * - Extracts numbered plan steps from "Plan:" sections
 * - [DONE:n] markers to complete steps during execution
 * - Progress tracking widget during execution
 * - Session-state restore is fail-closed (utils.ts: restorePlanModeState)
 *
 * See README.md for attribution — tool-policy.ts and question-tool.ts were ported from
...
import { extractTodoItems, markCompletedSteps, restorePlanModeState, type TodoItem } from "./utils.ts";

// Tools
const PLAN_MODE_TOOLS = ["read", "bash", "grep", "find", "ls", PLAN_MODE_QUESTION_TOOL_NAME];
...
export default function planModeExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			if (!answers) {
				return planModeQuestionCancelled(parsed.questions, "cancelled", "Plan-mode question cancelled by user.");
			}
...
Use the plan_mode_question tool to ask clarifying questions.
...
After completing a step, include a [DONE:n] tag in your response.`;
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (planModeEntry?.data !== undefined) {
			const restored = restorePlanModeState(planModeEntry.data);
			planModeEnabled = restored.enabled;
			todoItems = restored.todos;
			executionMode = restored.executing;
