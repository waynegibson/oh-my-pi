/**
 * Plan mode's own clarifying-questions tool.
 *
 * Ported from github.com/narumiruna/pi-extensions (extensions/pi-plan-mode/src/question-tool.ts).
 * Replaces a reference to an external "questionnaire" tool this extension didn't own (undocumented,
 * untyped coupling — would silently degrade if that tool were renamed or removed elsewhere in Pi).
 */
...
export function normalizePlanModeQuestionParams(input: unknown): NormalizePlanModeQuestionParamsResult {
  ...
  for (const [questionIndex, rawQuestion] of input.questions.entries()) {
    ...
    for (const [optionIndex, rawOption] of rawQuestion.options.entries()) {
      ...
      if (!description) {
        return { ok: false, error: `question ${questionIndex + 1} option ${optionIndex + 1} requires a description` };
      }
...
function formatPlanModeQuestionChoice(option: PlanModeQuestionOption, index: number): string {
...
export function planModeQuestionCancelled(questions: PlanModeQuestion[], reason: PlanModeQuestionReason, message: string) {
  return {
