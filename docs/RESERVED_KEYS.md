...
`ctrl+x` is **no longer safe** as of pi v0.80.6 — it's reserved for `app.models.clearAll` (scoped to the `/scoped-models` selector, but `registerShortcut()` still warns/conflicts on it globally). `theme-cycler.ts` moved off it onto `f2`.

This table is a snapshot and can drift from the installed pi version — the authoritative, always-current list is `docs/keybindings.md` in the installed package (`pi -v` to find the version, then check its `libexec/lib/node_modules/@earendil-works/pi-coding-agent/docs/keybindings.md` on Homebrew installs).
